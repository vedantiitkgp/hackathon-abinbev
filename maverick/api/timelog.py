...
class timelogfun:
    def add_employee(fname, lname, email, EmployeeID):
        ...
        res={'msg':response.text,'data':NULL}
        return res
    ...
    def all_employees():
        ...
        res={'msg':NULL,'data':pd.read_json(response.text)[['First Name','Last Name','Email ID','recordId','EmployeeID','Employee Role']]}
        return res
    ...
    def search_employee(email):
        ...
        response=requests.post(url)
        res={'msg':NULL,'data':pd.read_json(response.text)[['First Name','Last Name','Email ID','EmployeeID','Employee Role']]}
        return res
    ...
    def all_jobs(value):
        ...
        res={'msg':NULL,'data':pd.DataFrame(pd.read_json(response.text).iloc[1,0])[['jobName','jobStatus','fromDate','jobId']]}
        return res
    ...
    def get_timelog(user,fromDate,toDate):
        ...
        res={'msg':NULL,'data':pd.DataFrame(pd.read_json(response.text).iloc[1,0])[['employeeMailId','employeeFirstName','workDate','timelogId','jobName','jobIsActive','jobIsCompleted','hours']]}
        return res
