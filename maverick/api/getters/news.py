...
class news_fun:
	...
	def news_intent(self, params):
		if not params:
			...
			data= pd.DataFrame(data.groupby(['category'], as_index=False).first())[['title', 'category', 'url']]
			output = {'msg': 'I have fetched latest news for you.', 'data': data.to_dict('list')}
			return output
		else:
			t= "https://newsapi.org/v2/top-headlines?country=in&category=" + params['category'] + "&apiKey=d2f436851a964bac8a47fd50be902da6"
